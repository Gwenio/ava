...
class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			const {args, buildTitle, hookOptions, implementations, rawTitle} = parseTestArgs(testArgs);
	...
	async runHooks(tasks, contextRef, hookOptions, titleSuffix) {
		const hooks = tasks.map(task => new Runnable({
			contextRef,
			experiments: this.experiments,
			failWithoutAssertions: false,
			fn: t => task.implementation.call(null, {...t, options: hookOptions}, ...task.args),
	...
	async runTest(task, contextRef) {
		...
		const {hookOptions} = task;

		if (await this.runHooks(this.tasks.beforeEach, contextRef, hookOptions, hookSuffix)) {
			...
			if (result.passed) {
				...
				hooksAndTestOk = await this.runHooks(this.tasks.afterEach, contextRef, hookOptions, hookSuffix);
			} else {
				this.emit('stateChange', {
					type: 'test-failed',
					title: result.title,
		...
		const alwaysOk = await this.runHooks(this.tasks.afterEachAlways, contextRef, hookOptions, hookSuffix);
